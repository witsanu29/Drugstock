...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("
        UPDATE sub_warehouse
        SET receive_date = ?, quantity = ?
        WHERE id = ?
    ");

    $stmt->bind_param("sii", $receive_date, $quantity, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: sub_warehouse.php?success=1");
    exit;
}
...
.btn-primary:hover{
    ...
    box-shadow:0 6px 15px rgba(253,126,20,.35);
}
...
.form-select:focus{
    border-color:#fd7e14;
    box-shadow:0 0 0 .15rem rgba(253,126,20,.25);
}
